...
class BiquSpider(scrapy.Spider):
        ...
        try:
            ...
            if response.status == 200:
                cover = response.xpath(
                    '//div[@class="cover"]/img').re('.*?"(.*?)".*?alt')
                novel_name = response.xpath(
                    '//div[@class="info"]//h2/text()').get()
                author_ = response.xpath(
                    '//div[@class="small"]/span/text()').get()
                chapter_url = response.xpath(
                    '//div[@class="listmain"]//dd[position()>6]').re('.*?href="(.*?)">.*?</a>')
                type = response.xpath(
                    '//div[@class="small"]/span[2]/text()').get()
                info_ = response.xpath('//div[@class="intro"]/text()').get()
                UPtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())
                num = response.xpath(
                    '//div[@class="path"]/div').re('.*?k/(.*?)/')
                author = author_[3:]
                info = info_[3:]
                type = type[3:]
                ...
                for content in range(len(chapter_url)):
                    urls = '{url}{chapter_url}'.format(
                        url=self.paqu_urls, chapter_url=chapter_url[content])
                    items['chapter_id'] = content
                    yield scrapy.Request(url=urls,
                                         callback=self.parse_hapet,
                                         meta={'items': items, 'novelName': novel_name})

                next_url = '{url}/book/{num}'.format(
                    url=self.paqu_urls, num=int(num[0]) + 1)
                yield scrapy.Request(url=next_url)
            elif response.status == 404:
                next_url = '{url}/book/{num}'.format(
                    url=self.paqu_urls, num=int(num[0]) + 1)
                self.logger.info('url访问404，尝试下一个' + next_url)
                scrapy.Request(url=next_url)
            else:
    ...
    def parse_hapet(self, response):
        ...
        chapter_contents = response.xpath(
            '//div[@id="content"]').re('.*?showtxt">(.*?)https')
        chapter_title = response.xpath(
            '//div[@class="content"]//h1/text()').get()
        chapter_content = re.sub('<br>', '\n', chapter_contents[0])
        items['mark'] = 0
        self.logger.info(
            '---正在爬取：' + response.meta['novelName'] + '的章节：' + chapter_title)
        # 构造字典
        items['chapter'] = {
